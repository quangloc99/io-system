...
#define MIB2SEC(mib) (((mib) << 10 << 10) / SECTOR_SIZE)
...
#define PART_LINUX_PRIMARY 0x83
#define PART_EXTENDED 0x05

static const size_t PRIMARY_PART_SIZE[4] = {MIB2SEC(10), MIB2SEC(25), MIB2SEC(15)};
// static const size_t LOGICAL_PART_SIZE[] = {MIB2SEC(20), MIB2SEC(20)}; 

static void copy_mbr(u8 *disk, PartTable* def_part_table_ptr)
{
	memset(disk, 0x0, MBR_SIZE);
	*(unsigned long *)(disk + MBR_DISK_SIGNATURE_OFFSET) = 0x36E5756D;
	memcpy(disk + PARTITION_TABLE_OFFSET, def_part_table_ptr, PARTITION_TABLE_SIZE);
...
{
    PartTable def_part_table = {
        {
            // the other fields are not important since there is no real disk to read from or write to
            part_type: PART_LINUX_PRIMARY,
            abs_start_sec: 0x1,
            sec_in_part: PRIMARY_PART_SIZE[0] - 1,
        },
        {
            part_type: PART_LINUX_PRIMARY,
            abs_start_sec: PRIMARY_PART_SIZE[0],
            sec_in_part: PRIMARY_PART_SIZE[1],
        },
        {
            part_type: PART_LINUX_PRIMARY,
            abs_start_sec: PRIMARY_PART_SIZE[0] + PRIMARY_PART_SIZE[1],
            sec_in_part: PRIMARY_PART_SIZE[2],
        }
    };
    // unsigned int def_log_part_br_abs_start_sector[] = { 
        // def_part_table[1].abs_start_sec, 
        // def_part_table[1].abs_start_sec + LOGICAL_PART_SIZE[0], 
    // }; 
    // PartTable def_log_part_table[] = { 
        // { 
            // { 
                // part_type: PART_LINUX_PRIMARY, 
                // abs_start_sec: 0x1, 
                // sec_in_part: LOGICAL_PART_SIZE[0] - 1, 
            // }, 
            // { 
                // part_type: PART_EXTENDED, 
                // abs_start_sec: LOGICAL_PART_SIZE[0], 
                // sec_in_part: LOGICAL_PART_SIZE[1], 
            // } 
        // }, 
        // { 
            // { 
                // part_type: PART_LINUX_PRIMARY, 
                // abs_start_sec: 0x1, 
                // sec_in_part: LOGICAL_PART_SIZE[1] - 1, 
            // } 
        // } 
    // }; 
	// int i; 
	copy_mbr(disk, &def_part_table);
    // printk(KERN_INFO"%zu\n", ARRAY_SIZE(def_log_part_table)); 
